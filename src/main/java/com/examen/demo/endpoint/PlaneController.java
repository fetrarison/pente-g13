...
public class PlaneController {
    ...
    public ResponseEntity<?> fitPlane(@RequestBody List<Point3D> points) {
        try {
            ...
            RansacPlaneFitter.RansacResult result = fitter.findDominantPlane(points, 2000, 0.05, Math.min(8, points.size()));

            Map<String, Object> response = new HashMap<>();
            response.put("inliers", result.inliers);
            response.put("outliers", points.stream()
                    .filter(p -> !result.inliers.contains(p))
                    .toList());
